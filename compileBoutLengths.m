...
epochCounts = struct();
epochCounts.wild_type = struct('sleep', zeros(0, numBins));
epochCounts.mutant = struct('sleep', zeros(0, numBins));
...
totalCounts = struct();
totalCounts.wild_type = struct('sleep', []);
totalCounts.mutant = struct('sleep', []);
...
for i = 1:length(csvFiles)
    ...
    try
        ...
        % Adjust import options based on CSV structure
        % Assuming the third column contains sleep bout durations
        if size(opts.VariableNames, 2) >= 3
            % Select only the third column (sleep bout duration)
            opts.SelectedVariableNames = opts.VariableNames(3);
            % Set variable type to numeric
            opts = setvartype(opts, opts.SelectedVariableNames{1}, 'double');
            ...
            if ~isempty(sleepData)
                sleepDurations = table2array(sleepData);
                % Remove NaN values
                sleepDurations = sleepDurations(~isnan(sleepDurations));
                
                fprintf('  Found %d sleep bouts\n', length(sleepDurations));
...
for genotype = {'wild_type', 'mutant'}
    ...
    if isfield(epochCounts, gen)
        % Calculate mean and std for sleep bouts
...
% Create figures for sleep bouts
stateLabel = 'Sleep';

%% 1. Line Plot
figure('Name', [stateLabel ' Bout Durations - Line Plot'], 'Position', [100, 100, 800, 500]);
hold on;

% Plot wild-type data with error bars
if isfield(meanCounts, 'wild_type') && all(isfinite(meanCounts.wild_type.sleep))
    errorbar(1:numBins, meanCounts.wild_type.sleep, semCounts.wild_type.sleep, ...
        'Color', wtColor, 'LineWidth', 2, 'Marker', 'o', 'MarkerFaceColor', wtColor, 'MarkerSize', 8);
end

% Plot mutant data with error bars
if isfield(meanCounts, 'mutant') && all(isfinite(meanCounts.mutant.sleep))
    errorbar(1:numBins, meanCounts.mutant.sleep, semCounts.mutant.sleep, ...
        'Color', mutColor, 'LineWidth', 2, 'Marker', 'o', 'MarkerFaceColor', mutColor, 'MarkerSize', 8);
end

% Add labels and legend
title([stateLabel ' Bout Durations']);
...
% Save the figure
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Line.fig']));
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Line.png']));

%% 2. Bar Plot
figure('Name', [stateLabel ' Bout Durations - Bar Plot'], 'Position', [100, 100, 800, 500]);
hold on;
...
% Plot wild-type bars
if isfield(meanCounts, 'wild_type') && all(isfinite(meanCounts.wild_type.sleep))
    barHandles(1) = bar(wtPos, meanCounts.wild_type.sleep, barWidth, 'FaceColor', wtColor);
    
    % Add error bars
    errorbar(wtPos, meanCounts.wild_type.sleep, stdCounts.wild_type.sleep, '.k');
end

% Plot mutant bars
if isfield(meanCounts, 'mutant') && all(isfinite(meanCounts.mutant.sleep))
    barHandles(2) = bar(mutPos, meanCounts.mutant.sleep, barWidth, 'FaceColor', mutColor);
    
    % Add error bars
    errorbar(mutPos, meanCounts.mutant.sleep, stdCounts.mutant.sleep, '.k');
end
...
% Save the figure
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Bar.fig']));
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Bar.png']));

%% 3. Dot Plot (Individual mice with mean)
figure('Name', [stateLabel ' Bout Durations - Dot Plot'], 'Position', [100, 100, 800, 500]);
hold on;
...
if isfield(epochCounts, 'wild_type') && ~isempty(epochCounts.wild_type.sleep)
    for i = 1:size(epochCounts.wild_type.sleep, 1)
        scatter(1:numBins, epochCounts.wild_type.sleep(i,:), 50, wtColor, 'o', 'filled', 'MarkerFaceAlpha', 0.3);
    end
    
    % Plot mean with error bars and capture handle
    h_wt = errorbar(1:numBins, meanCounts.wild_type.sleep, semCounts.wild_type.sleep, ...
...
if isfield(epochCounts, 'mutant') && ~isempty(epochCounts.mutant.sleep)
    for i = 1:size(epochCounts.mutant.sleep, 1)
        scatter((1:numBins)+0.2, epochCounts.mutant.sleep(i,:), 50, mutColor, 'o', 'filled', 'MarkerFaceAlpha', 0.3);
    end
    
    % Plot mean with error bars and capture handle
    h_mut = errorbar((1:numBins)+0.2, meanCounts.mutant.sleep, semCounts.mutant.sleep, ...
...
% Save the figure
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Dot.fig']));
saveas(gcf, fullfile(outputFolder, [stateLabel '_Bouts_Dot.png']));
